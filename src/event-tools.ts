  ...
  server.registerTool(
    'register',
    {
      ...
      description:
        ...
        '`dueAt`, `decisions[]` and `trace` (optional; ignored when comparing an idempotent retry, so a Milestone ' +
        're-sent with a different `trace` still dedupes); Verdict accepts `claim`, `source`, `result`, `evidence`, ' +
        '`target` (`hex:target:<id>`), `supersedes[]`, `origin` and `trace`. `milestoneType: "gate"` and the ' +
        '`gate` key are reserved for the gate Milestone generated by `evaluate_gate`.',
      inputSchema: {
        project: Name,
        process: Name,
