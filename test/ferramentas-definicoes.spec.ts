...
describe('M2', () => {
  ...
  const CASOS: { tool: string; campo: string; base: Record<string, unknown> }[] = [
    { tool: 'criar_processo', campo: 'projeto', base: { projeto: 'proj-valido', processo: 'proc-valido' } },
...
describe('M7', () => {
  test('registrar_tipo com schema reprovado pelo Ajv → SCHEMA_INVALIDO estruturado', async () => {
    const resultado = await ambiente.chamar('registrar_tipo', { projeto: 'p1', nome: 'ruim', schema: { typ: 'object' } });
  ...
  test('registrar_tipo com schema: [] → Input validation error (forma), sem chegar ao handler', async () => {
    const resultado = await ambiente.chamar('registrar_tipo', { projeto: 'p1', nome: 'ruim', schema: [] });
    expect(resultado.isError).toBe(true);
    expect(resultado.content?.[0]?.text).toMatch(/^Input validation error/);
  });
  ...
  test('handler que lança exceção interna devolve INTERNO estruturado, nunca a exceção crua', async () => {
    ...
    const logs: Registro[] = [];
    const ctxTeste = { dirDados: ambiente.dir, relogio: () => new Date(), log: (r: Registro) => logs.push(r) };

    servidorTeste.registerTool(
      'explode',
      { description: 'lança exceção interna de propósito', inputSchema: {}, outputSchema: { ok: z.boolean() } },
      async () =>
    ...
    await Promise.all([servidorTeste.connect(transporteServidor), clienteTeste.connect(transporteCliente)]);
    ...
    expect(logs.some((registro) => registro.evento === 'erro-interno' && typeof registro.stack === 'string')).toBe(true);
...
describe('M8', () => {
  test('listar {processo} não traz schema dos tipos; listar {tipo} traz o schema fixado', async () => {
    ...
    const tipos = (processo.structuredContent as { processo: { tipos: { nome: string; hash: string; schema?: unknown }[] } })
      .processo.tipos;
    expect(tipos).toEqual([{ nome: 'nota', hash: (tipo.structuredContent as { hash: string }).hash }]);
    expect(tipos[0]).not.toHaveProperty('schema');

    const doTipo = await ambiente.chamar('listar', { projeto: 'p1', processo: 'proc1', tipo: 'nota' });
    expect(doTipo.structuredContent).toEqual({
      gatesEmbutidos: (doTipo.structuredContent as { gatesEmbutidos: unknown }).gatesEmbutidos,
      tipo: { nome: 'nota', hash: (tipo.structuredContent as { hash: string }).hash, schema: SCHEMA_VALIDO },
    });
  });
});
...
describe('M9', () => {
  test('annotations das 5 tools de definição batem com §4.12', async () => {
    ...
    expect(porNome.listar).toMatchObject({ readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false });
    for (const nome of ['registrar_tipo', 'registrar_vocabulario', 'registrar_gate']) {
      expect(porNome[nome]).toMatchObject({ readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: false });
    }
...
describe('S1', () => {
  test('registrar_tipo grava schemas/<nome>.json com o schema exato', async () => {
    ...
    const gravado = JSON.parse(fs.readFileSync(path.join(ambiente.dir, 'p1', 'schemas', 'nota.json'), 'utf8'));
  ...
  test('rejeita schema inválido, $ref externo e nomes reservados, sem gravar', async () => {
    ...
    const invalido = await ambiente.chamar('registrar_tipo', { projeto: 'p1', nome: 'ruim', schema: { typ: 'object' } });
    esperarErro(invalido, 'SCHEMA_INVALIDO');

    const refExterno = await ambiente.chamar('registrar_tipo', {
      projeto: 'p1',
      nome: 'comref',
      schema: { type: 'object', properties: { x: { $ref: 'https://exemplo.invalido/schema.json' } } },
    });
    esperarErro(refExterno, 'SCHEMA_INVALIDO');

    const marco = await ambiente.chamar('registrar_tipo', { projeto: 'p1', nome: 'marco', schema: SCHEMA_VALIDO });
    esperarErro(marco, 'NOME_RESERVADO');

    const veredito = await ambiente.chamar('registrar_tipo', { projeto: 'p1', nome: 'veredito', schema: SCHEMA_VALIDO });
...
describe('S8', () => {
  ...
  test('registrar_gate com nome de gate embutido → NOME_RESERVADO', async () => {
    for (const nome of ['sem-orfaos', 'sem-conflitos', 'cadeia-integra', 'sem-referencias-invalidas']) {
      const resultado = await ambiente.chamar('registrar_gate', { projeto: 'p1', nome, criterio: 'x' });
      esperarErro(resultado, 'NOME_RESERVADO');
    }
  });
});
...
describe('N14', () => {
  ...
  test('criar_processo sem nenhum vocabulário registrado → VOCABULARIO_AUSENTE', async () => {
    const resultado = await ambiente.chamar('criar_processo', { projeto: 'sem-vocab', processo: 'proc1' });
...
describe('listar', () => {
  test('sem parâmetros: projetos existentes com a contagem de processos, e gatesEmbutidos sempre presente', async () => {
    ...
    const corpo = resultado.structuredContent as { projetos: { nome: string; processos: number }[]; gatesEmbutidos: unknown[] };
  ...
  test('processo sem projeto, ou tipo sem processo → ENTRADA_INVALIDA', async () => {
    ...
    esperarErro(await ambiente.chamar('listar', { projeto: 'p1', tipo: 'nota' }), 'ENTRADA_INVALIDA');
  });
  ...
  test('processo inexistente → PROCESSO_INEXISTENTE', async () => {
    await ambiente.chamar('registrar_vocabulario', { projeto: 'p1', dono: 'nucleo' });
    esperarErro(await ambiente.chamar('listar', { projeto: 'p1', processo: 'fantasma' }), 'PROCESSO_INEXISTENTE');
  });

  test('tipo fora do snapshot do processo → TIPO_INEXISTENTE', async () => {
    await prepararProcesso(ambiente, 'p1', 'proc1');
    esperarErro(await ambiente.chamar('listar', { projeto: 'p1', processo: 'proc1', tipo: 'fantasma' }), 'TIPO_INEXISTENTE');
  });
  ...
  test('ignora diretórios reservados e diretórios de processo sem manifesto', async () => {
    ...
    const processos = (resultado.structuredContent as { projeto: { processos: { nome: string }[] } }).projeto.processos;
...
describe('logger', () => {
  test('toda chamada gera um registro tool com nome e ms; erro também traz codigo', async () => {
    ...
    const registroSucesso = ambiente.registros.find((r) => r.evento === 'tool' && r.nome === 'listar');
    ...
    const chamadasListar = ambiente.registros.filter((r) => r.evento === 'tool' && r.nome === 'listar');
