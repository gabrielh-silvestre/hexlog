...
function dadosVeredito(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return { afirmacao: 'a', fonte: 'f', resultado: 'ok', prova: 'p', destino: 'hex:alvo:u1', origem: 'o', rastro: 'r', ...overrides };
}
...
function lerManifesto(ambiente: Ambiente, projeto: string, processo: string): unknown {
  return JSON.parse(fs.readFileSync(path.join(ambiente.dir, projeto, processo, 'processo.json'), 'utf8'));
}

function escreverLog(ambiente: Ambiente, projeto: string, processo: string, linhas: (Linha | string)[]): void {
  const texto = linhas.map((linha) => (typeof linha === 'string' ? linha : JSON.stringify(linha))).join('\n') + '\n';
...
describe('M2', () => {
  ...
  const CASOS: { tool: string; campo: string; base: Record<string, unknown> }[] = [
    { tool: 'registrar', campo: 'projeto', base: { projeto: PROJ, processo: PROC, id: PREFIXO_MARCO, agente: AGENTE, dados: dadosMarco() } },
    { tool: 'registrar', campo: 'processo', base: { projeto: PROJ, processo: PROC, id: PREFIXO_MARCO, agente: AGENTE, dados: dadosMarco() } },
    { tool: 'avaliar_gate', campo: 'projeto', base: { projeto: PROJ, processo: PROC, gate: 'sem-orfaos', agente: AGENTE, alvo: 'hex:alvo:u1' } },
    { tool: 'avaliar_gate', campo: 'processo', base: { projeto: PROJ, processo: PROC, gate: 'sem-orfaos', agente: AGENTE, alvo: 'hex:alvo:u1' } },
...
describe('M3', () => {
  const CASOS: { tool: string; args: Record<string, unknown> }[] = [
    { tool: 'registrar', args: { projeto: PROJ, processo: 'fantasma', id: `${PROJ}:fantasma:marco`, agente: AGENTE, dados: dadosMarco() } },
    { tool: 'avaliar_gate', args: { projeto: PROJ, processo: 'fantasma', gate: 'sem-orfaos', agente: AGENTE, alvo: 'hex:alvo:u1' } },
...
describe('M7', () => {
  test('avaliar_gate com alvo fora do formato hex:alvo: → Input validation error', async () => {
    ...
    const resultado = await ambiente.chamar('avaliar_gate', { projeto: PROJ, processo: PROC, gate: 'sem-orfaos', agente: AGENTE, alvo: 'u1' });
    expect(resultado.isError).toBe(true);
    expect(resultado.content?.[0]?.text).toMatch(/^Input validation error/);
  });
  ...
  test('registrar com id fora da gramática → ID_INVALIDO estruturado', async () => {
    await preparar(ambiente, PROJ, PROC);
    const resultado = await ambiente.chamar('registrar', { projeto: PROJ, processo: PROC, id: 'lixo', agente: AGENTE, dados: dadosMarco() });
...
describe('M8', () => {
  test('eventos pagina 250 linhas em 3 páginas de até 100, em ordem, sem exceder o teto de caracteres', async () => {
    ...
    for (;;) {
      ...
      const corpo = resultado.structuredContent as { eventos: Linha[]; proximoCursor: number | null };
      paginas.push(corpo);
  ...
  test('estado com 150 vigentes → 100 itens na lista e totais.vigentes = 150', async () => {
    ...
    const corpo = resultado.structuredContent as { vigentes: unknown[]; totais: Record<string, number> };
...
describe('M9', () => {
  test('annotations das 5 tools de eventos batem com §4.12', async () => {
    ...
    for (const nome of ['registrar', 'avaliar_gate']) {
      expect(porNome[nome]).toMatchObject({ readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false });
    }
...
describe('N1', () => {
  ...
  test('(a) JSON inválido na linha 1', async () => {
    ...
    escreverLog(ambiente, PROJ, PROC, [linhas[0]!, '{ json quebrado', linhas[2]!, linhas[3]!, linhas[4]!]);
  ...
  test('(b) texto livre alterado em dados da linha 1', async () => {
    const linhas = await logDe5();
    const alterada: Linha = { ...linhas[1]!, dados: { ...linhas[1]!.dados, marcoTipo: 'adulterado' } };
  ...
  test('(d) bytes parciais sem \\n no fim + novo registrar → repara e encadeia', async () => {
    ...
    const registrado = await ambiente.chamar('registrar', { projeto: PROJ, processo: PROC, id: PREFIXO_MARCO, agente: AGENTE, dados: dadosMarco() });
  ...
  test('(e) (a) + prevHash alterado na linha 4', async () => {
    ...
    escreverLog(ambiente, PROJ, PROC, [linhas[0]!, '{ json quebrado', linhas[2]!, linhas[3]!, linha4Alterada]);
    const cadeia = await chamarCadeia();
  ...
  test('(f) lixo com \\n anexado ao fim + registrar legítimo → repara', async () => {
    ...
    await ambiente.chamar('registrar', { projeto: PROJ, processo: PROC, id: PREFIXO_MARCO, agente: AGENTE, dados: dadosMarco() });
...
describe('N2', () => {
  test('(i) Marco com prazoExecucao em offset reenviado igual → deduplicado, mesmo seq, sem nova linha', async () => {
    ...
    const primeiro = await ambiente.chamar('registrar', { projeto: PROJ, processo: PROC, id: PREFIXO_MARCO, agente: AGENTE, dados });
    const corpo1 = primeiro.structuredContent as { evento: Linha };
    const antes = ambiente.arvore();

    const segundo = await ambiente.chamar('registrar', { projeto: PROJ, processo: PROC, id: corpo1.evento.id, agente: AGENTE, dados });
  ...
  test('(ii) tipo custom com default omitido, reenviado omitido ou explícito → deduplicado', async () => {
    await preparar(ambiente, PROJ, PROC);
    const primeiro = await ambiente.chamar('registrar', { projeto: PROJ, processo: PROC, id: PREFIXO_NOTA, agente: AGENTE, dados: { nota: 'x' } });
    const corpo1 = primeiro.structuredContent as { evento: Linha };

    const reenviadoOmitido = await ambiente.chamar('registrar', { projeto: PROJ, processo: PROC, id: corpo1.evento.id, agente: AGENTE, dados: { nota: 'x' } });
  ...
  test('conteúdo diferente com o mesmo id completo → ID_CONFLITANTE', async () => {
    await preparar(ambiente, PROJ, PROC);
    const primeiro = await ambiente.chamar('registrar', { projeto: PROJ, processo: PROC, id: PREFIXO_MARCO, agente: AGENTE, dados: dadosMarco() });
    const corpo1 = primeiro.structuredContent as { evento: Linha };
...
describe('N4', () => {
  ...
  test('resultado fora do vocabulário → grava e devolve aviso VOCABULARIO_DESCONHECIDO', async () => {
    ...
    expect(corpo.avisos).toEqual(expect.arrayContaining([expect.objectContaining({ codigo: 'VOCABULARIO_DESCONHECIDO' })]));
  });
});
...
describe('N5', () => {
  test('sem-orfaos: estado limpo passa, Marco vencido reprova com prova', async () => {
    await preparar(ambiente, PROJ, PROC);
    const limpo = await ambiente.chamar('avaliar_gate', { projeto: PROJ, processo: PROC, gate: 'sem-orfaos', agente: AGENTE, alvo: 'hex:alvo:u1' });
    ...
    const violado = await ambiente.chamar('avaliar_gate', { projeto: PROJ, processo: PROC, gate: 'sem-orfaos', agente: AGENTE, alvo: 'hex:alvo:u2' });
  ...
  test('cadeia-integra passa num log íntegro', async () => {
    await preparar(ambiente, PROJ, PROC);
    const resultado = await ambiente.chamar('avaliar_gate', { projeto: PROJ, processo: PROC, gate: 'cadeia-integra', agente: AGENTE, alvo: 'hex:alvo:u1' });
...
describe('N6', () => {
  test('gate custom sem resultado → AVALIACAO_INVALIDA', async () => {
    await preparar(ambiente, PROJ, PROC);
    const resultado = await ambiente.chamar('avaliar_gate', { projeto: PROJ, processo: PROC, gate: 'gate-custom', agente: AGENTE, alvo: 'hex:alvo:u1' });
  ...
  test('gate registrado depois de criar_processo → GATE_NAO_REGISTRADO', async () => {
    await preparar(ambiente, PROJ, PROC);
    await ambiente.chamar('registrar_gate', { projeto: PROJ, nome: 'gate-tardio', criterio: 'critério tardio' });
    const resultado = await ambiente.chamar('avaliar_gate', {
      projeto: PROJ,
      processo: PROC,
      gate: 'gate-tardio',
  ...
  test('gate custom aceito → Marco de gate com criterio do snapshot e origem custom', async () => {
    ...
    const dados = corpo.evento.dados as { marcoTipo: string; gate: { nome: string; origem: string; criterio: string; passou: boolean } };
...
describe('N8', () => {
  test('timestamp termina em Z; prazoExecucao com offset é normalizado para UTC', async () => {
    ...
    expect((corpo.evento.dados as { prazoExecucao: string }).prazoExecucao).toBe('2026-09-16T21:00:00.000Z');
  });
});
...
describe('N13', () => {
  test('Marco vencido gera órfão; avaliar_gate não o remove de estado.orfaos', async () => {
    ...
    const antes = await ambiente.chamar('estado', { projeto: PROJ, processo: PROC, secoes: ['orfaos'] });
    expect((antes.structuredContent as { orfaos: unknown[] }).orfaos).toHaveLength(1);

    const avaliado = await ambiente.chamar('avaliar_gate', { projeto: PROJ, processo: PROC, gate: 'sem-orfaos', agente: AGENTE, alvo: 'hex:alvo:x' });
    expect((avaliado.structuredContent as { passou: boolean }).passou).toBe(false);

    const depois = await ambiente.chamar('estado', { projeto: PROJ, processo: PROC, secoes: ['orfaos'] });
  ...
  test('Marco de gate sozinho num alvo não cria abertura', async () => {
    await preparar(ambiente, PROJ, PROC);
    await ambiente.chamar('avaliar_gate', { projeto: PROJ, processo: PROC, gate: 'sem-orfaos', agente: AGENTE, alvo: 'hex:alvo:y' });

    ambiente.definirRelogio(new Date('2099-01-01T00:00:00.000Z'));
    const resultado = await ambiente.chamar('estado', { projeto: PROJ, processo: PROC, secoes: ['orfaos'] });
...
describe('N14', () => {
  ...
  test('fixado alterado com hashes recalculados → cadeia.quebras inclui {0, hash-nao-bate}', async () => {
    ...
    const cadeia = (await ambiente.chamar('cadeia', { projeto: PROJ, processo: PROC })).structuredContent as Cadeia;
    expect(cadeia.quebras).toEqual(expect.arrayContaining([{ indice: 0, motivo: 'hash-nao-bate' }]));
  });
});
...
describe('S3', () => {
  test('eventos custom aparecem em eventos e ficam inertes na projeção do Estado', async () => {
    ...
    await ambiente.chamar('registrar', { projeto: PROJ, processo: PROC, id: PREFIXO_NOTA, agente: AGENTE, dados: { nota: 'intercalado' } });
    await ambiente.chamar('registrar', {
      projeto: PROJ,
      processo: PROC,
      id: PREFIXO_VEREDITO,
    ...
    const eventos = (await ambiente.chamar('eventos', { projeto: PROJ, processo: PROC })).structuredContent as { eventos: Linha[] };
    expect(eventos.eventos.map((evento) => evento.tipo)).toEqual(['marco', 'nota', 'veredito']);

    const estado = (await ambiente.chamar('estado', { projeto: PROJ, processo: PROC })).structuredContent as {
      vigentes: unknown[];
      orfaos: unknown[];
      conflitos: unknown[];
      aRevisar: unknown[];
...
describe('S5', () => {
  test('schema, vocabulário e gate alterados entre a criação de dois processos: cada um usa sua versão', async () => {
    const projeto = 'p-s5';
    const schemaAntigo = { type: 'object', properties: { nota: { type: 'string' } }, required: ['nota'], additionalProperties: false };
    const schemaNovo = {
      type: 'object',
    ...
    await ambiente.chamar('registrar_vocabulario', { projeto, dono: 'nucleo', marcoTipo: ['v1'], resultado: [], acao: [] });
    ...
    await ambiente.chamar('registrar_vocabulario', { projeto, dono: 'nucleo', marcoTipo: ['v2'], resultado: [], acao: [] });
